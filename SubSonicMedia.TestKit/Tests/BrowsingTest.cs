...
namespace SubSonicMedia.TestKit.Tests
{
    ...
    public class BrowsingTest : TestBase
    {
        ...
        protected override async Task<TestResult> ExecuteTestAsync()
        {
            ...
            try
            {
                var foldersResponse = await Client.Browsing.GetMusicFoldersAsync();
                RecordTestResult(foldersResponse, "browsing_music_folders");
                ...
                {
                    ConsoleHelper.LogError($"Failed to get music folders: {foldersResponse.Error?.Message}");
            ...
            try
            {
                var artistsResponse = await Client.Browsing.GetArtistsAsync();
                RecordTestResult(artistsResponse, "browsing_artists");
                ...
                if (artistsResponse.IsSuccess)
                {
                    ...
                    if (artistsResponse.Artists?.Index != null && artistsResponse.Artists.Index.Count > 0)
                    {
                        var firstIndex = artistsResponse.Artists.Index.FirstOrDefault();
                        if (firstIndex?.Artist != null && firstIndex.Artist.Count > 0)
                        {
                            ConsoleHelper.LogInfo($"Sample artists from index '{firstIndex.Name}':");
                ...
                {
                    ConsoleHelper.LogError($"Failed to get artists: {artistsResponse.Error?.Message}");
                    allTestsPassed = false;
                }
            }
            catch (Exception ex)
            ...
            if (allTestsPassed)
            {
                ...
                try
                {
                        ...
                        artistsResponse.Artists.Index.Count > 0)
                    {
                        ...
                        var firstArtist = artistsResponse.Artists.Index
                            .SelectMany(i => i.Artist ?? new List<Responses.Browsing.Artist>())
                            .FirstOrDefault(a => !string.IsNullOrEmpty(a.Id));
                            
                        if (firstArtist != null)
                        {
                            ConsoleHelper.LogInfo($"Getting details for artist: {firstArtist.Name}");
                            
                            var artistResponse = await Client.Browsing.GetArtistAsync(firstArtist.Id!);
                            RecordTestResult(artistResponse, "browsing_artist_detail");
                            
                            if (artistResponse.IsSuccess && artistResponse.Artist != null)
                            {
                                int albumCount = artistResponse.Artist.Album?.Count ?? 0;
                                ConsoleHelper.LogSuccess($"Successfully retrieved {albumCount} albums for {firstArtist.Name}");
                            ...
                            {
                                ConsoleHelper.LogError($"Failed to get artist details: {artistResponse.Error?.Message}");
                                allTestsPassed = false;
                            }
                        }
                        else
                        {
                            ConsoleHelper.LogWarning("No artists with valid IDs found to test GetArtist");
                        }
                    }
                }
                catch (Exception ex)
