...
namespace SubSonicMedia.TestKit.Tests
{
    ...
    public class BookmarkTest : TestBase
    {
        ...
        protected override async Task<TestResult> ExecuteTestAsync()
        {
            ...
            throw new SubsonicApiException(70, "The bookmark function is not implemented on this server");
            ...
            try
            {
                var bookmarksResponse = await Client.Bookmarks.GetBookmarksAsync();
                RecordTestResult(bookmarksResponse, "bookmarks_list");
                ...
                {
                    ConsoleHelper.LogError($"Failed to get bookmarks: {bookmarksResponse.Error?.Message}");
            ...
            if (allTestsPassed)
            {
                ...
                try
                {
                        ...
                        !string.IsNullOrEmpty(randomSongsResponse.RandomSongs.Song[0].Id))
                    {
                        ...
                        var createResponse = await Client.Bookmarks.CreateBookmarkAsync(
                            song.Id,
                            position,
                        ...
                        {
                            ConsoleHelper.LogSuccess($"Successfully created bookmark for song: {song.Title}");
                            
                            // Delete the bookmark afterward
                            ConsoleHelper.LogInfo($"Deleting bookmark for song: {song.Title}");
                            var deleteResponse = await Client.Bookmarks.DeleteBookmarkAsync(song.Id);
                            ...
                            {
                                ConsoleHelper.LogError($"Failed to delete bookmark: {deleteResponse.Error?.Message}");
                                allTestsPassed = false;
                            }
                        }
                        else
                        {
                            ConsoleHelper.LogError($"Failed to create bookmark: {createResponse.Error?.Message}");
                            allTestsPassed = false;
                        }
                    }
                    else
                    {
                        ConsoleHelper.LogWarning("No media files available to test bookmark creation");
                    }
                }
                catch (Exception ex)
                {
                    ConsoleHelper.LogError($"Error testing bookmark creation/deletion: {ex.Message}");
                    allTestsPassed = false;
                }
            }
