...
namespace SubSonicMedia.TestKit.Tests
{
    ...
    public class SystemTest : TestBase
    {
        ...
        public override string Description =>
            "Tests system features including ping, license, and scan status";
        ...
        protected override async Task<TestResult> ExecuteTestAsync()
        {
            ...
            try
            {
                var pingResponse = await this.Client.System.PingAsync();
                this.RecordTestResult(pingResponse, "system_ping");

                if (pingResponse.IsSuccess)
                {
                    ConsoleHelper.LogSuccess(
                        $"Successfully pinged server, version: {pingResponse.Version}"
                    );
                }
                else
                {
            ...
            try
            {
                var licenseResponse = await this.Client.System.GetLicenseAsync();
                this.RecordTestResult(licenseResponse, "system_license");
                ...
                if (licenseResponse.IsSuccess)
                {
                    ...
                    table.AddRow(
                        "License Version",
                        licenseResponse.License.LicenseVersion ?? "N/A"
                    );
                ...
                else
                {
                    ConsoleHelper.LogError(
                        $"Failed to get license: {licenseResponse.Error?.Message}"
                    );
                    allTestsPassed = false;
                }
            }
            ...
            try
            {
                var scanStatusResponse = await this.Client.System.GetScanStatusAsync();
                this.RecordTestResult(scanStatusResponse, "system_scan_status");
                ...
                {
                    ConsoleHelper.LogError(
                        $"Failed to get scan status: {scanStatusResponse.Error?.Message}"
                    );
                    allTestsPassed = false;
                }
            }
            ...
            if (runScanTest && allTestsPassed)
            {
                ...
                try
                {
                    var startScanResponse = await this.Client.System.StartScanAsync();
                    ...
                    if (startScanResponse.IsSuccess)
                    {
                        ...
                        var checkStatusResponse = await this.Client.System.GetScanStatusAsync();

                        if (checkStatusResponse.IsSuccess)
                        {
                            ConsoleHelper.LogInfo(
                                $"Scan in progress: {checkStatusResponse.ScanStatus.Scanning}"
                            );

                            if (checkStatusResponse.ScanStatus.Scanning)
                            {
                                ConsoleHelper.LogInfo(
                                    $"Items scanned so far: {checkStatusResponse.ScanStatus.Count}"
                                );

                                if (!string.IsNullOrEmpty(checkStatusResponse.ScanStatus.Folder))
                                {
                                    ConsoleHelper.LogInfo(
                                        $"Currently scanning folder: {checkStatusResponse.ScanStatus.Folder}"
                                    );
                                }
                            }
                        }
                        ...
                        {
                            ConsoleHelper.LogError(
                                $"Failed to check scan status: {checkStatusResponse.Error?.Message}"
                            );
                        }
                    }
                    else
                    {
                        ConsoleHelper.LogError(
                            $"Failed to start scan: {startScanResponse.Error?.Message}"
                        );
                        allTestsPassed = false;
                    }
                }
