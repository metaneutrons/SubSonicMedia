...
namespace SubSonicMedia.TestKit.Tests
{
    ...
    public class JukeboxTest : TestBase
    {
        ...
        protected override async Task<TestResult> ExecuteTestAsync()
        {
            ...
            try
            {
                ...
                )
                {
                    ...
                    ConsoleHelper.LogSuccess(
                        $"Found song: {randomSongsResponse.RandomSongs.Song[0].Title} by {randomSongsResponse.RandomSongs.Song[0].Artist}"
                    );
                }
                else
                {
            ...
            try
            {
                var statusResponse = await this.Client.Jukebox.GetJukeboxStatusAsync();
                this.RecordTestResult(statusResponse, "jukebox_status");
                ...
                if (statusResponse.IsSuccess)
                {
                    ...
                    table.AddRow(
                        "Current Index",
                        statusResponse.JukeboxStatus.CurrentIndex.ToString()
                    );
                ...
                else
                {
                    ConsoleHelper.LogError(
                        $"Failed to get jukebox status: {statusResponse.Error?.Message}"
                    );
            ...
            if (allTestsPassed && !string.IsNullOrEmpty(songId))
            {
                ...
                try
                {
                    ...
                    var setResponse = await this.Client.Jukebox.SetJukeboxPlaylistAsync(
                    ...
                    if (setResponse.IsSuccess)
                    {
                        ...
                        var playlistResponse = await this.Client.Jukebox.GetJukeboxStatusAsync();
                        this.RecordTestResult(playlistResponse, "jukebox_playlist");

                        if (playlistResponse.IsSuccess)
                        {
                            int entryCount = playlistResponse.JukeboxStatus?.Entry?.Count ?? 0;
                            ConsoleHelper.LogSuccess(
                                $"Retrieved jukebox playlist with {entryCount} entries"
                            );
                            ...
                            var controlResponse = await this.Client.Jukebox.ControlJukeboxAsync(
                                "start"
                            );
                            ...
                            if (controlResponse.IsSuccess)
                            {
                                ...
                                var stopResponse = await this.Client.Jukebox.ControlJukeboxAsync(
                                    "stop"
                                );
                                if (stopResponse.IsSuccess)
                                {
                                    ConsoleHelper.LogSuccess(
                                        "Successfully stopped jukebox playback"
                                    );
                                }
                                else
                                {
                                    ConsoleHelper.LogError(
                                        $"Failed to stop jukebox playback: {stopResponse.Error?.Message}"
                                    );
                                }
                            }
                            else
                            {
                                ConsoleHelper.LogError(
                                    $"Failed to start jukebox playback: {controlResponse.Error?.Message}"
                                );
                            }

                            // Clear the jukebox playlist
                            ConsoleHelper.LogInfo("Clearing jukebox playlist...");
                            var clearResponse = await this.Client.Jukebox.SetJukeboxPlaylistAsync(
                                new List<string>()
                            );
                            ...
                            {
                                ConsoleHelper.LogError(
                                    $"Failed to clear jukebox playlist: {clearResponse.Error?.Message}"
                                );
                                allTestsPassed = false;
                            }
                        }
                        ...
                        {
                            ConsoleHelper.LogError(
                                $"Failed to get jukebox playlist: {playlistResponse.Error?.Message}"
                            );
                            allTestsPassed = false;
                        }
                    }
                    ...
                    {
                        ConsoleHelper.LogError(
                            $"Failed to set jukebox playlist: {setResponse.Error?.Message}"
                        );
                        allTestsPassed = false;
                    }
                }
